...
The plot below illustrates the data aggregated from one subject's non-drusen IS/OS. In order to calculate the angle correctly, the x and z sampling densities of the OCT image must be considered. The width of the OCT scan on the retina was $2 mm$, sampled with 800 A-scans, resulting in a pixel width of $2.5 \mu m$. The depth corresponding to a single pixel was $5 \mu m$, which requires a scaling of the angle by a factor of 2. Assuming an eye length of $22.7 mm$ and refractive index of $1.33$ (thus an optical path length through the eye of $16.7 mm$), a tangent angle $\theta$, and eye length $\ell$, the effective pupil position $x$ is given by:
...
x = np.tan(angle)*16.67
print(x.min(),x.max())
amplitude = angle_amplitude[:,1]


plt.figure(figsize=(3.7,3),dpi=150)
plt.plot(x,amplitude,'.',alpha=0.2,markersize=1)
...
![png](dOCT_drusen_example_files/dOCT_drusen_example_3_1.png)
...
def rolling_median(pupil_position,amp,window_width=1.0,step_size=0.5,diagnostics=False):
    ...
    for t in np.arange(t_start,t_end+step_size,step_size):
        ...
        if len(idx)>0:
            ...
            amplitude_mean.append(np.median(amp[idx]))
...
window_centers, amplitude_mean, amplitude_std = rolling_median(x,amplitude)
plt.figure(figsize=(3.7,3),dpi=150)
# plot the raw data with low alpha
plt.plot(x,amplitude,'.',alpha=0.1,markersize=1,label='raw data')
# plot the average with standard deviation bars
plt.errorbar(window_centers,amplitude_mean,amplitude_std,label='rolling median')
plt.xlabel('effective pupil position (mm)')
